...
void SignVerifyMessageDialog::on_signMessageButton_SM_clicked()
{
    ...
    CBitcoinLamboAddress addr(ui->addressIn_SM->text().toStdString());
    if (!addr.IsValid()) {
...
{
    CBitcoinLamboAddress addr(ui->addressIn_VM->text().toStdString());
    if (!addr.IsValid()) {
    ...
    if (!(CBitcoinLamboAddress(pubkey.GetID()) == addr)) {
        ui->statusLabel_VM->setStyleSheet("QLabel { color: red; }");

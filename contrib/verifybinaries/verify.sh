...
###   bitcoinlambocore.org and bitcoinlambo.org and compares them.
...
WORKINGDIR="/tmp/bitcoinlambo_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://bitcoinlambocore.org"
HOST2="https://bitcoinlambo.org"
BASEDIR="/bin/"
VERSIONPREFIX="bitcoinlambo-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'bitcoinlambo-',
...
if [ $? -ne 0 ]; then
   echo "bitcoinlambo.org failed to provide signature file, but bitcoinlambocore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "bitcoinlambo.org and bitcoinlambocore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the BitcoinLambo Core binary release signing key installed?"
   fi

...
The RPM spec file provided here is for BitcoinLambo-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://bitcoinlambo.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[BitcoinLambo git repository](https://github.com/bitcoinlambo/bitcoinlambo) but are not part of
...
Currently only `Source10` is used, for the example `bitcoinlambo.conf` file.
...
in the bitcoinlambo git repository. At present only an SVG version of the BitcoinLambo
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the bitcoinlambo
...
needed when 0.12.1 is released, a proper fix is already in the BitcoinLambo git
master branch.
...
idea to build BitcoinLambo Core against a static version of that library compiled
...
The `bitcoinlambo.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `bitcoinlambo-server` package. At this point, most RPM based
...
The `bitcoinlambo-server` package should have SELinux support. How to properly do
...
BitcoinLambo-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/bitcoinlambo/). He has been
packaging BitcoinLambo for Fedora at least since 2012.
...
4. I renamed the `bitcoinlambo` package that contains the Qt GUI to `bitcoinlambo-core` as
that appears to be how the general population refers to it, in contrast to
`bitcoinlambo-xt` or `bitcoinlambo-classic`. I wanted to make sure the general population
